    ...
    <div class="title">${msg("activity-stats.header")}</div>
   	...
   	<div class="body activity-stats-body" id="${args.htmlid}-body">
	    ...
	    <#if user.isAdmin>
                <div class="detail-list-item first-item last-item">
	    	    <br/>
	    	    <img src="${page.url.context}/res/alfea/components/dashlets/activity-stats/email_users.gif" style="vertical-align:top"/>&nbsp;&nbsp;<a href="mailto:<#list connectedUsers as connectedUser>${connectedUser.email}<#if connectedUser_has_next>;</#if></#list>">${msg("activity-stats.send-email", connectedUsersCount)}</a> 
	        </div>
            </#if>
